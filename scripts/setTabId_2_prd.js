...
async function updateDocuments(dbName, updateData) {
  ...
  const snapshot = await collectionRef.where("userId", "==", "RSzGJ1om5iZMMvcmYBgThDhzpjc2").get();
...
taskDbs.forEach((dbName) =>
  updateDocuments(dbName,  { tabId: 'oYL4pIMfSnPj89SFP5YJ' }).catch(console.error)
);
...
purchaseDbs.forEach((dbName) =>
  updateDocuments(dbName,  { tabId: 'oycy7tRzr40Tu3P91hPh' }).catch(console.error)
);
